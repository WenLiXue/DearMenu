...
| 2026-04-08 | v4.0 | UI全面优化 + 老公端情感化重设计 |
...
## v4.0 UI 全面优化（2026-04-08）

### 核心变更

| 变更 | 说明 |
|------|------|
| 布局统一化 | 统一使用 `100dvh` 动态视口高度 |
| TabBar 添加订单入口 | 5标签固定底部：首页、点餐、订单、收藏、我的 |
| 情绪化动画 | 心跳、飘浮、渐隐动画 |
| 紧凑布局模式 | 首页减少留白，提高信息密度 |

### 新增/修改文件

| 文件 | 变更 |
|------|------|
| frontend/src/components/Layout.tsx | TabBar 添加订单入口 |
| frontend/src/theme.css | 新增情绪动画（heartbeat、float、fadeIn） |
| frontend/src/pages/Home.tsx | 紧凑三列布局，移除邀请码 |
| frontend/src/pages/Home.css | 紧凑模式样式 |
| frontend/src/pages/Profile.tsx | 添加邀请码生成功能 |
| backend/routes/orders.py | 添加 DELETE 接口 |
| frontend/src/pages/HusbandPage/* | 老公端情感化重设计组件 |

### 老公端情感化重设计

| 组件 | 说明 |
|------|------|
| HeroCard.tsx | 主任务卡片，放大设计、背景渐变 |
| PendingTasks.tsx | 待做任务折叠区 |
| ProgressBar.tsx | 粗进度条 + 渐变 + 动画 |
| EmotionModule.tsx | 成就文案、关系化文案 |
| FeedbackModal.tsx | 完成反馈弹窗 |

### 情绪设计要点

- **问候语**：根据时段变化（早上/中午/下午/晚上）
- **成就文案池**："今天为她做了一顿 💪"、"她看到一定会很开心"
- **完成动效**：卡片向上滑出 + 进度条填充动画
- **偷懒反馈**："好耶！躺平～ 😴"
